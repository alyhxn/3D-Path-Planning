...
class Simulator{
    ...
    initGUI(){
        ...
        endFolder.add(this.endObj, 'x', 1, 49, 1)
        endFolder.add(this.endObj, 'y', 1, 49, 1)
        endFolder.add(this.endObj, 'z',1, 49, 1)
        ...
        settings.add(this.props, 'Algo', [ 'Astar', 'RRT', 'RRT_Ext', 'RBT', 'RBT_Ext', 'RRRT']);
            ...
            if(self.delta % self.FPS == 0)
                if(self.check){
                    if(self.props.Simulation){
                        ...
                        for(let i=0;i<20;i++){
                            let count = 0
                            data += "\nNo of Obstacles,"+self.props.No_of_obs+"\nAstar,,RRT,,RBT\nTime,Cost,Nodes,Time,Cost,Nodes,Time,CostNodes\n";
                            let cost;
                            const algos = ['Astar', 'RRT', 'RBT_Ext'];
                            while(count<100){
                                for(const item of algos){
                                    const startTime = new Date();
                                    self.props.Algo = item;
                                    self.onRun();
                                    cost = self.algo.run();
                                    const time = new Date() - startTime + ' ms'
                                    console.log(time, cost);
                                    data += time + "," + cost + "," + self.algo.nodes.length + ",";
                                }
                                data += "\n";
                                count++;
                            ...
                            self.props.No_of_obs += 100
                            self.onApply();
                        }
                        const file = new File([data], "test.csv", {type: "text/plain;charset=utf-8"});
                        saveAs(file)
                        self.check = false;
                    }
